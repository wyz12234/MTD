...
def process_agent_data(agent_fut, distance_th=0.5):
    num_agent, time_length, _ = agent_fut.shape
    
    # 初始化新的数组
    processed_data = torch.zeros((num_agent, time_length, 3))
    ...
    for agent in range(num_agent):
        ...
        for t in range(time_length):
            x, y, cos, sin, v = agent_fut[agent, t, :5]
            
            if torch.isnan(x) or torch.isnan(y) or torch.isnan(cos) or torch.isnan(sin):
                if t == 0 or not initial_valid:
                    continue
                else:
                    # 后续时间点为NaN，用最后一个有效值补齐
                    processed_data[agent, t] = last_valid_value
            else:
                if not initial_valid:
                    initial_valid = True
                    initial_pos = torch.tensor([x, y])
                
                yaw = torch.atan2(sin, cos)
                processed_data[agent, t] = torch.tensor([x, y, yaw])
                last_valid_value = processed_data[agent, t]

        if initial_valid:
            end_pos = processed_data[agent, time_length - 1, :2]
            distance = torch.dist(initial_pos, end_pos)
            if distance < distance_th:
                processed_data[agent] = torch.zeros((time_length, 3))
        else:
            # 如果初始数据无效，整个agent数据置为全零
            processed_data[agent] = torch.zeros((time_length, 3))
...
def guided_rollout(
    ...
    while not done:
        ...
        with timers.timed("obs"):
            ...
            if first_scene_ts is None:
                first_scene_ts = obs['agents']['scene_ts']
            obs['agents']['first_scene_ts'] = first_scene_ts
            if guided_pos is not None:
                obs['agents']['guided_pos'] = guided_pos
                obs['agents']['guided_yaw'] = guided_yaw
            elif eval_class == 'STRIVE' or eval_class == 'SceneDiffuser':
                agent_fut = process_agent_data(torch.tensor(obs['agents']['agent_fut']), distance_th=0.0)
                pos_pred_global, yaw_pred_global = transform_agents_to_world(agent_fut[..., :2].unsqueeze(1), agent_fut[..., 2:].unsqueeze(1), 
                                                                             torch.tensor(obs['agents']["world_from_agent"]))
                obs['agents']['guided_pos'] = pos_pred_global.squeeze(1).numpy()
                obs['agents']['guided_yaw'] = yaw_pred_global[:, 0, :, 0].numpy()
        ...
        with timers.timed("env_step"):
            ...
            if env._metrics['all_collision_rate'].coll_ts is not None and new_guidance_config is None:
                new_guidance_config = [guidance_config[0][1:]]
                if guidance_config[0][0]['name'] == 'adv_loss':
                    policy_model.set_guidance(new_guidance_config, ex_obs['agents'])
        if render:
            frames.extend(ims)
        keys_to_keep = ['target_positions', 'agent_from_world', 'world_from_agent', 'history_positions', 'history_yaws', 'extent', 'history_speeds', 'history_availabilities', 'extras', 'scene_ids', 'first_scene_ts']
        obs['agents'] = {key: obs['agents'][key] for key in keys_to_keep if key in obs['agents']}
        save_obs.append(obs)
    ...
    # policy.agents_policy.policy.model.adversary
    ...
    return metrics, env_info, renderings, save_obs, added_metrics
...
def heuristic_adversarial(sim_scene, dt, guide_moving_speed_th, crash_min_infront, crash_min_t, **kwargs):
    guide_config = {
        'name' : 'adv_loss',
        'params' : {
                    'guide_moving_speed_th' : guide_moving_speed_th,
                    'crash_min_infront' : crash_min_infront,
                    'crash_min_t' : crash_min_t,
                    },
        'agents' : None, # all agents
    }
    return guide_config

def heuristic_followtrajection(sim_scene, dt, decay_rate, horizon, guide_moving_speed_th, **kwargs):
    guide_config = {
        'name' : 'follow_traj',
        'params' : {
                    'decay_rate' : decay_rate, 
...
def heuristic_matchplanner(sim_scene, dt, decay_rate, horizon, guide_moving_speed_th, is_adv, **kwargs):
    guide_config = {
        'name' : 'match_planner',
        'params' : {
                    'decay_rate' : decay_rate, 
...
def heuristic_strive(sim_scene, dt, guide_moving_speed_th, crash_min_infront, crash_min_t, **kwargs):
    guide_config = {
        'name' : 'strive',
        'params' : {
                    'guide_moving_speed_th' : guide_moving_speed_th,
...
HEURISTIC_FUNC = {
    ...
    'adv_loss' : heuristic_adversarial,
    'follow_traj' : heuristic_followtrajection,
    'match_planner' : heuristic_matchplanner,
    'strive' : heuristic_strive,
}
...
def scene_to_video(rasterizer, scene_data, scene_name, output_dir,
    ...
    for i, frame_i in enumerate(frames):
        ...
        if first_frame_only:
            ffn = os.path.join(output_dir, "{sname}_{simnum:04d}_{framei:03d}.svg").format(sname=scene_name, simnum=sim_num, framei=frame_i)
        else:
