...
from utils.guidance_utils import check_behind
...
class AgentCollisionLoss(GuidanceLoss):
    ...
    def forward(self, x, data_batch, agt_mask=None):
        '''
            x : (B, N, T, 6) where 6 is (x, y, vel, yaw, acc, yawvel)
        '''
        agt_mask = agt_mask.clone()
        data_extent = data_batch["extent"]
        data_world_from_agent = data_batch["world_from_agent"]
        ...
        stationary = ~moving
        stationary = stationary.view(-1, 1, 1, 1).expand_as(x).clone()
        stationary[0] = False
        x_masked = torch.where(stationary, x.detach(), x)

        adversary_mask = data_batch.get('adversary_mask', None)
        if adversary_mask is not None:
            adversary_mask_matrix = torch.zeros_like(x_masked).to(torch.bool)
            adversary_mask_matrix[adversary_mask, torch.arange(x_masked.size(1))] = True
            x_masked = torch.where(adversary_mask_matrix, x_masked.detach(), x_masked)
            agt_mask[0] = False

        
        B = x.shape[0]
        T = x.shape[2]
        T_total = data_batch.get('T', None)
        if T_total is not None:
            # mask T_total
            if T_total < T:
                time_mask = torch.arange(T, device=x_masked.device) < T_total
                agent_mask = torch.arange(B, device=x_masked.device) > 0
                agent_mask = agent_mask.view(B, 1, 1)
                combined_mask = torch.logical_and(agent_mask, time_mask)
                x_masked = torch.where(combined_mask.unsqueeze(-1), x_masked.detach(), x_masked)
            else:
                agent_mask = (torch.arange(B, device=x_masked.device) > 0)
                x_masked = torch.where(agent_mask.view(B, 1, 1, 1), x_masked.detach(), x_masked)

        pos_pred = x_masked[..., :2]
        yaw_pred = x_masked[..., 3:4]
        ...
        if self.centroids is None or self.penalty_dists is None:
            centroids, agt_rad = self.init_disks(self.num_disks, data_extent) # B x num_disks x 2
            # minimum distance that two vehicle circle centers can be apart without collision (+ buffer)
            penalty_dists = agt_rad.view(N,B, 1).expand(B, B) + agt_rad.view(1, B).expand(B, B) + self.buffer_dist
        else:
...
class SceneAgentCollisionLoss(GuidanceLoss):
    '''
    Agents should not collide with each other.
    NOTE: this assumes full control over the scene. 
    '''
    ...
    def init_disks(self, num_disks, extents):
        ...
        # 对每个批次和每个元素，在 x 轴上采样圆盘的中心点
        cent_x = torch.stack([
            torch.stack([torch.linspace(cent_min[b, vidx].item(), cent_max[b, vidx].item(), num_disks)
                        for vidx in range(NA)], dim=0)
            for b in range(batch_size)
        ], dim=0).to(extents.device)

        # 生成中心点，并沿第3维堆叠x轴和y轴坐标
        centroids = torch.stack([cent_x, torch.zeros_like(cent_x)], dim=3)

        return centroids, agt_rad

    # TODO why are results when using num_scenes_per_batch > 1 different than = 1?
    def init_mask(self, batch_scene_index, device):
        batch_size = batch_scene_index.size(0)  # 获取批次大小
        scene_mask_list = []  # 用于存储每个批次的场景掩码

        for b in range(batch_size):
            # 处理每个批次
            _, data_scene_index = torch.unique_consecutive(batch_scene_index[b], return_inverse=True)
            scene_block_list = []
            scene_inds = torch.unique_consecutive(data_scene_index)
            ...
            # 为当前批次创建场景掩码
            batch_scene_mask = torch.block_diag(*scene_block_list).to(device)
            scene_mask_list.append(batch_scene_mask)
        
        # 将所有批次的掩码堆叠起来，形成最终的掩码
        scene_mask = torch.stack(scene_mask_list)
    ...
    def forward(self, x, data_batch, agt_mask=None):
        ...
        # 扩展 agt_mask 形状以匹配 pos_pred_global 和 yaw_pred_global
        agt_mask_expanded = agt_mask.view(B, -1, T, 1)  # (B, N) -> (B, N, 1, 1)
        ...
        pos_pred_global = torch.where(agt_mask_expanded, pos_pred_global, pos_pred_global.detach())
        yaw_pred_global = torch.where(agt_mask_expanded, yaw_pred_global, yaw_pred_global.detach())    

        # create disks and transform to world frame (centroids)
        N, B, T, _ = pos_pred_global.size()
        if self.centroids is None or self.penalty_dists is None:
            centroids, agt_rad = self.init_disks(self.num_disks, data_extent) # B x num_disks x 2
            # minimum distance that two vehicle circle centers can be apart without collision (+ buffer)
            penalty_dists = agt_rad.view(N, B, 1).expand(N, B, B) + agt_rad.view(N, 1, B).expand(N, B, B) + self.buffer_dist
        else:
            centroids, penalty_dists = self.centroids, self.penalty_dists
        centroids = centroids.unsqueeze(2).expand(N, B, T, self.num_disks, 2)
        # to world
        s = torch.sin(yaw_pred_global).unsqueeze(-1)
        c = torch.cos(yaw_pred_global).unsqueeze(-1)
        rotM = torch.cat((torch.cat((c, s), dim=-1), torch.cat((-s, c), dim=-1)), dim=-2)
        ...
        centroids = centroids.transpose(1,2) # T x NS x B x D x 2
        centroids = centroids.reshape((T*N, B, self.num_disks, 2))
        # distances between all pairs of circles between all pairs of agents
        cur_cent1 = centroids.view(T*N, B, 1, self.num_disks, 2).expand(T*N, B, B, self.num_disks, 2).reshape(T*N*B*B, self.num_disks, 2)
        cur_cent2 = centroids.view(T*N, 1, B, self.num_disks, 2).expand(T*N, B, B, self.num_disks, 2).reshape(T*N*B*B, self.num_disks, 2)
        ...
        penalty_dists = penalty_dists.view(N, B, B).unsqueeze(1).expand(N, T, B, B).reshape(T*N, B, B)
        scene_mask = scene_mask.unsqueeze(1).expand(N, T, B, B).reshape(T*N, B, B)
        is_colliding_mask = torch.logical_and(
            pair_dists <= penalty_dists,  # 计算两个张量之间的比较，返回布尔张量
            scene_mask  # 直接使用生成的 scene_mask
        )

        agt_coll_mask = agt_mask.permute(0, 2, 1).reshape(T*N, -1).unsqueeze(-1) * agt_mask.permute(0, 2, 1).reshape(T*N, -1).unsqueeze(-2)
        is_colliding_mask = is_colliding_mask & agt_coll_mask
        ...
        # summing over timesteps and all other agents to get B x N
        cur_penalties = cur_penalties.reshape((N, T, B, B)).transpose(0, 1)
        # cur_penalties = cur_penalties.sum(0).sum(-1).transpose(0, 1)
        # penalize early steps more than later steps
        exp_weights = torch.tensor([self.decay_rate ** t for t in range(T)], device=cur_penalties.device)
        exp_weights /= exp_weights.sum()
...
class MapCollisionLoss(GuidanceLoss):
    ...
    def __init__(self, num_points_lw=(10, 10), decay_rate=0.9, guide_moving_speed_th=5e-1):
        ...
        '''
        super().__init__()
        self.num_points_lw = num_points_lw # (10, 10)
        self.decay_rate = decay_rate
        self.guide_moving_speed_th = guide_moving_speed_th
    ...
    def forward(self, x, data_batch, agt_mask=None):   
        ...
        moving = torch.abs(data_batch['curr_speed']) > self.guide_moving_speed_th
        moving_mask = moving.view((B,1,1)).expand(B, N, T).clone()
        moving_mask[0] = False
        ...
        if T_total is not None:
            # mask T_total
            if T_total < T:
                time_mask = torch.arange(T, device=all_coll_loss.device) < T_total
                agent_mask = torch.arange(B, device=all_coll_loss.device) > 0
                agent_mask = agent_mask.view(B, 1, 1)
                combined_mask = torch.logical_and(agent_mask, time_mask)
                all_coll_loss = torch.where(combined_mask, torch.zeros_like(all_coll_loss), all_coll_loss)
            else:
                agent_mask = torch.arange(B, device=all_coll_loss.device) > 0
                all_coll_loss = torch.where(agent_mask.view(B, 1, 1), torch.zeros_like(all_coll_loss), all_coll_loss)
...
class SceneMapCollisionLoss(GuidanceLoss):
    '''
    Agents should not go offroad.
    '''
    def __init__(self, num_points_lw=(10, 10), decay_rate=0.9, guide_moving_speed_th=5e-1):
    ...
    def forward(self, x, data_batch, agt_mask=None):   
        ...
        _, H, W, _ = drivable_map.size()


        B, N, T, d = x.size()

        if agt_mask is not None:
            agt_mask_expanded = agt_mask.view(B, N, T, 1).expand(B, N, T, d)

            # 使用 torch.where 进行条件操作
            x = torch.where(agt_mask_expanded, x.detach(), x)
        traj = x.reshape((-1, d)) # B*N*T x 6
        pos_pred = traj[:,:2]
        yaw_pred = traj[:, 3:4] 

        lw = data_extent.unsqueeze(2).expand((B, N, T, 3)).reshape((-1, 3))[:,:2]
        diag_len = torch.sqrt(torch.sum(lw*lw, dim=-1))
        data_raster_from_agent = data_raster_from_agent.unsqueeze(2).expand((B, N, T, 3, 3)).reshape((-1, 3, 3))
        ...
        # query these points in the drivable area to determine collision
        _, P, _ = agt_samp_pts.size()
        map_coll_mask = torch.isclose(batch_detect_off_road(torch.flatten(agt_samp_pix, start_dim=0, end_dim=1), torch.flatten(drivable_map, start_dim=0, end_dim=1)), torch.ones((1)).to(agt_samp_pix.device))
        map_coll_mask = map_coll_mask.reshape((-1, P))

        # only apply loss to timesteps that are partially overlapping
        per_step_coll = torch.sum(map_coll_mask, dim=-1)
        overlap_mask = ~torch.logical_or(per_step_coll == 0, per_step_coll == P)
        overlap_mask = overlap_mask & agt_mask.view(-1)
...
class KeepDistanceLoss2(GuidanceLoss):
    ...
    def forward(self, x, data_batch, agt_mask=None):
        ...
        # penalize early steps more than later steps[torch.arange(x_masked.size(0)), adversary_mask] 
...
class AdversarialLoss(GuidanceLoss):
    def __init__(self, guide_moving_speed_th=5e-1, crash_min_infront=0.0, crash_min_t=0):
        super().__init__()
        self.guide_moving_speed_th = guide_moving_speed_th
        self.crash_min_infront = crash_min_infront
        self.crash_min_t = crash_min_t
    ...
    def forward(self, x, data_batch, agt_mask=None):
        ...
        ego_pos = pos_pred_global[0, :, self.crash_min_t:, :].detach() # N x T x 2
        # ego_yaw = yaw_pred_global[0, :, self.crash_min_t:, :] # N x T x 1
        agent_pos = pos_pred_global[1:, :, self.crash_min_t:, :].transpose(0, 1) # N x B-1 x T x 2
        agent_yaw = yaw_pred_global[1:, :, self.crash_min_t:, :].transpose(0, 1) # N x B-1 x T x 1
        ego_pos_expand = ego_pos.unsqueeze(1).expand_as(agent_pos) # N x B-1 x T x 2
        dist_traj = torch.norm(ego_pos_expand - agent_pos, dim=-1) # N x B-1 x T
        min_dist_in  = dist_traj

        # check_behind
        behind_steps = check_behind(ego_pos_expand.detach(), agent_pos.detach(), agent_yaw.detach(), self.crash_min_infront) # N x B-1 x T
        behind_traj = torch.sum(behind_steps, dim=-1, keepdim=True) == behind_steps.size(-1) # N x B-1 x 1
        behind_traj = behind_traj.expand_as(behind_steps) # N x B-1 x T
        # 取或
        behind_traj = (~moving)[1:].unsqueeze(0).unsqueeze(-1).expand_as(behind_traj) | behind_traj
        all_true_per_matrix = torch.all(behind_traj, dim=(1, 2)) # N
        if all_true_per_matrix.any(): # every agent is behind... just optim normally for now
            behind_traj_clone = behind_traj.clone()  # 克隆扩展后的张量
            behind_traj_clone[all_true_per_matrix] = False
            behind_traj = behind_traj_clone
        min_dist_in = torch.where(behind_traj, float('inf')*torch.ones_like(min_dist_in), min_dist_in) # N x B-1 x T
        N = min_dist_in.size(0)
        T = min_dist_in.size(-1) - self.crash_min_t
        min_dist = nn.functional.softmin(min_dist_in.reshape(N, -1), dim=-1) # N x B-1 * T exp

        if min_dist.shape[-1] == 0:
            return torch.zeros(N, dtype=torch.float32, device=min_dist_in.device) # no other agents
        
        cur_adversary_agent = (torch.max(min_dist, dim=-1)[1] // T) + 1 # N
        # cur_adversary_t = (torch.max(min_dist, dim=-1)[1] % T) + self.crash_min_t # N

        # 2. compute the loss
        dist_traj_adversary = dist_traj[torch.arange(0, cur_adversary_agent.shape[0]), cur_adversary_agent - 1, :] # N x T
        weight_for_adversary = nn.functional.softmin(dist_traj_adversary, dim=-1) # N x T
        adv_loss = ((dist_traj_adversary ** 2) * weight_for_adversary).sum(-1) # N x T
        if any(torch.max(min_dist, dim=-1)[0]): 
            data_batch['adversary_mask'] = cur_adversary_agent
        
        # 3. normalize the loss
        # normal_loss = (0.5 * (x.reshape(N, -1) - x_baseline.reshape(N, -1)) ** 2).sum(-1)
        

        # adv_loss = ((dist_traj ** 2).reshape(N, -1) * min_dist).sum(-1)
        return adv_loss
        # return adv_loss + normal_loss

class FollowTrajectoryLoss(GuidanceLoss):
    def __init__(self, decay_rate=0.9, horizon=52, guide_moving_speed_th=5e-1):
        super().__init__()
        self.decay_rate = decay_rate
    ...
    def forward(self, x, data_batch, agt_mask=None):
        ...
        agent_pos = pos_pred_global[1:].transpose(0, 1) # N x B-1 x T x 2
        agent_yaw = yaw_pred_global[1:].transpose(0, 1) # N x B-1 x T x 1

        guided_pos = data_batch['guided_pos'][1:] # B-1 x T_total x 2
        guided_yaw = data_batch['guided_yaw'][1:].unsqueeze(-1) # B-1 x T_total x 1

        if guided_pos.shape[1] < self.global_t + 1 + self.horizon:
            guided_pos = guided_pos[:, self.global_t + 1:, :]
            guided_yaw = guided_yaw[:, self.global_t + 1:, :]
        else:
            guided_pos = guided_pos[:, self.global_t + 1: self.global_t + 1 + self.horizon, :]
            guided_yaw = guided_yaw[:, self.global_t + 1: self.global_t + 1 + self.horizon, :]
        T = guided_pos.shape[1]
        N = agent_pos.shape[0]
        data_batch['T'] = T

        guided_pos_expand = guided_pos.unsqueeze(0).repeat(N, 1, 1, 1) # N x B-1 x T x 2
        guided_yaw_expand = guided_yaw.unsqueeze(0).repeat(N, 1, 1, 1) # N x B-1 x T x 1

        # l2 loss
        exp_weights = torch.tensor([self.decay_rate ** t for t in range(T)], device=x.device)
        exp_weights /= exp_weights.sum()
        pos_dev = torch.norm(agent_pos[:, :, :T] - guided_pos_expand, dim=-1) # N x B-1 x T
        yaw_dev = torch.abs(agent_yaw[:, :, :T] - guided_yaw_expand).squeeze(-1) # N x B-1 x T
        total_dev = (pos_dev ** 2) * exp_weights[None, None, :] + (yaw_dev ** 2) * exp_weights[None, None, :]

        traj_loss = total_dev.sum(-1).sum(-1) # N

        return traj_loss


class MatchPlannerLoss(GuidanceLoss):
    def __init__(self, decay_rate=1.0, horizon=52, guide_moving_speed_th=5e-1, is_adv=False):
        super().__init__()
        self.decay_rate = decay_rate
        self.horizon = horizon
        self.guide_moving_speed_th = guide_moving_speed_th
    ...
    def forward(self, x, data_batch, agt_mask=None):
        ...
        stationary = stationary.view(-1, 1, 1, 1).expand_as(x).clone()
        stationary[0] = False
        x_masked = torch.where(stationary, x.detach(), x) # B x N x T x 6
        if self.is_adv is not None:
            if self.is_adv:
                masked = torch.ones_like(x_masked).bool()
                masked[0] = False
            else:
                masked = torch.zeros_like(x_masked).bool()
                masked[0] = True
            x_masked = torch.where(masked, x_masked, x_masked.detach())
        ...
        agent_pos = pos_pred_global.transpose(0, 1) # N x B x T x 2
        agent_yaw = yaw_pred_global.transpose(0, 1) # N x B x T x 1

        guided_pos = data_batch['guided_pos'] # B x T_total x 2
        guided_yaw = data_batch['guided_yaw'].unsqueeze(-1) # B x T_total x 1

        T = guided_pos.shape[1]
        N = agent_pos.shape[0]

        guided_pos_expand = guided_pos.unsqueeze(0).repeat(N, 1, 1, 1) # N x B x T x 2
        guided_yaw_expand = guided_yaw.unsqueeze(0).repeat(N, 1, 1, 1) # N x B x T x 1
        ...
        if self.is_adv is not None:
            if self.is_adv:
                loss_mask = torch.ones_like(total_dev).bool()
                loss_mask[:, 0, :] = False
            else:
                loss_mask = torch.zeros_like(total_dev).bool()
                loss_mask[:, 0, :] = True
            total_dev = torch.where(loss_mask, torch.zeros_like(total_dev), total_dev)

        traj_loss = total_dev.sum(-1).sum(-1) # N

        return traj_loss

class STRIVELoss(GuidanceLoss):
    def __init__(self, guide_moving_speed_th=5e-1, crash_min_infront=0.0, crash_min_t=0):
        super().__init__()
        self.guide_moving_speed_th = guide_moving_speed_th
        self.crash_min_infront = crash_min_infront
    ...
    def forward(self, x, data_batch, agt_mask=None):
        ...
        # 2. compute the loss
        adv_loss = torch.sum(min_dist.view(N, -1) * (dist_traj ** 2).view(N, -1), dim=1)

        data_batch['adversary_mask'] = 0
        init_weight = torch.mean(torch.sum(min_dist.reshape(N, -1, T), dim=2), dim=0)
        data_batch['init_weight'] = torch.cat([torch.ones_like(init_weight[:1]), init_weight[0:]])
...
GUIDANCE_FUNC_MAP = {
    ...
    'follow_traj': FollowTrajectoryLoss,
    'match_planner': MatchPlannerLoss,
    'strive': STRIVELoss,
}
