    ...
    for ei in range(num_episodes):
        ...
        while not done:
            ...
            with timers.timed("obs"):
                ...
                if scene_data:
                    obs['agents']['action_traj_positions'] = scene_data['action_traj_positions']
                    obs['agents']['action_traj_yaws'] = scene_data['action_traj_yaws']
                    obs['agents']['action_sample_positions'] = scene_data['action_sample_positions']
                    obs['agents']['action_sample_yaws'] = scene_data['action_sample_yaws']
        ...
        other_metrics = {}
        for k, v in env._metrics['all_off_road_rate'].get_sol_episode_metrics(101).items():
            other_metrics['offroad_' + k] = v
        
        # all_comfort
        for k, v in env._metrics['all_comfort'].get_sol_episode_metrics(101).items():
            other_metrics['comfort_' + k] = v
    ...
    def _run_rollout(self, pl_module: pl.LightningModule, global_step: int):
        ...
        while scene_i < len(eval_scenes):
            ...
            stats, info, renderings, adjust_plans = rollout_episodes(
                env,
                rollout_policy,
                num_episodes=self._eval_cfg.num_episode_repeats, # 1
                n_step_action=self._eval_cfg.n_step_action, # 5
                render=self._save_video,
                skip_first_n=self._eval_cfg.skip_first_n, # 0
                scene_indices=scene_indices,
                start_frame_index_each_episode=self._eval_cfg.start_frame_index_each_episode, # null
                seed_each_episode=self._eval_cfg.seed_each_episode, # null
                horizon=self._eval_cfg.num_simulation_steps # 100
            )
