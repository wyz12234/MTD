...
guidance_keywords = ['map_names', 'curr_speed', 'history_positions', 'history_yaws', 'agent_from_world', 'world_from_agent', 'scene_index', 'extent', 'drivable_map', 'raster_from_agent', 'agent_hist', 'extras', 'guided_pos', 'guided_yaw']
...
def trajdata2posyawspeed(state, nan_to_zero=True): # state: [batch_size, num_agents, seq_len, state_dim]
...
def parse_scene_centric(batch: dict):
    ...
    centered_state = batch["centered_agent_state"] # 原始坐标系下的状态
    ...
    agent_hist_extent = batch["agent_hist_extent"] # l,w,h
    ...
    if batch["maps"] is not None:
        map_res = batch["maps_resolution"][0,0] # convert to pixels/meter
...
def convert_scene_data_to_agent_coordinates(data, merge_BM=False, max_neighbor_num=None, max_neighbor_dist=np.inf, keep_order_of_neighbors=False):
    ...
    # 将agent的位置和yaw转换到agent-centric坐标系
    ...
    for i in range(B):
        ...
        for j in range(M):
            chosen_neigh_inds = nb_idx[j][nb_idx[j]>=0].tolist() # 每个智能体的邻居

            # (Q. 3. 3)
            center_from_world = agent_from_world[j]
            world_from_neigh = world_from_agent[chosen_neigh_inds]
            center_from_neigh = center_from_world.unsqueeze(0) @ world_from_neigh # agent坐标系之间的矩阵
...
def add_scene_dim_to_agent_data(obs):
    ...
    for k in prep_keywords_for_interaction_edge+major_keywords:
        if isinstance(obs[k], torch.Tensor):
            ...
            new_obs[k] = [obs[k]]
    if 'guided_pos' in obs:
        new_obs['guided_pos'] = obs['guided_pos'].unsqueeze(0)
        new_obs['guided_yaw'] = obs['guided_yaw'].unsqueeze(0)
    if 'action_traj_positions' in obs:
        new_obs['action_traj_positions'] = obs['action_traj_positions'].unsqueeze(0)
        new_obs['action_traj_yaws'] = obs['action_traj_yaws'].unsqueeze(0)
        new_obs['action_sample_positions'] = obs['action_sample_positions'].unsqueeze(0)
        new_obs['action_sample_yaws'] = obs['action_sample_yaws'].unsqueeze(0)
