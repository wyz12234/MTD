...
class OffRoadRate(EnvMetrics):
    ...
    def add_step(self, state_info: dict, all_scene_index: np.ndarray):
        ...
        if self.coll_ts is None:
            self.coll_ts = state_info.get('coll_ts', None)
            self.coll_index = state_info.get('coll_index', None)
    ...
    def get_adv_episode_metrics(self, adv_index, adv_time):
        # 计算对抗时间前的offroad rate
        self._df.set_index(["scene_index","track_id","ts"])
        metric_by_agt = self._df[self._df["ts"] <= adv_time].groupby(["scene_index", "track_id"])["met"].sum()
        adv_frame = metric_by_agt[metric_by_agt.index.get_level_values("track_id") == adv_index].to_numpy()
        adv_rate = np.array([self._df[(self._df["ts"] <= adv_time) & (self._df["track_id"] == adv_index)]["met"].mean()])

        other_frame = np.array([metric_by_agt[~metric_by_agt.index.get_level_values("track_id").isin([adv_index, 0])].mean()])
        other_rate = np.array([self._df[(self._df["ts"] <= adv_time) & (~self._df["track_id"].isin([adv_index, 0]))]["met"].mean()])
    ...
    def get_sol_episode_metrics(self, adv_index):
        # 计算解决的专家优化轨迹的offroad rate
        self._df.set_index(["scene_index","track_id","ts"])
        metric_by_agt = self._df.groupby(["scene_index","track_id"])["met"].sum()
        ego_frame = metric_by_agt[metric_by_agt.index.get_level_values("track_id") == 0].to_numpy()
        ego_rate = np.array([self._df[self._df["track_id"] == 0]["met"].mean()])

        other_frame = np.array([metric_by_agt[~metric_by_agt.index.get_level_values("track_id").isin([adv_index, 0])].mean()])
        other_rate = np.array([self._df[~self._df["track_id"].isin([adv_index, 0])]["met"].mean()])
...
class CollisionRate(EnvMetrics):
    ...
    @staticmethod
    def compute_per_step(state_info: dict, all_scene_index: np.ndarray, self=None, ts=None):
        ...
        for i in range(num_scenes):
            ...
            for j in range(num_agents_in_scene):
                ...
                if coll is not None:
                    ...
                    if self is not None:
                        if j == 0 and self.coll_ts is None:
                            # 表明coll[1]与ego发生碰撞
                            self.coll_ts = ts[0]
                            self.coll_index = coll[1] + 1
                            state_info['coll_ts'] = self.coll_ts
                            state_info['coll_index'] = self.coll_index
    ...
    def add_step(self, state_info: dict, all_scene_index: np.ndarray):
        ...
        ts = np.array([self._scene_ts[sid] for sid in state_info["scene_index"]])
        met_all, _ = self.compute_per_step(state_info, all_scene_index, self, ts)
        step_df = []
        for k in met_all:
            if k=="coll_any":
                type=-1
    ...
    def get_adv_episode_metrics(self, adv_index, adv_time):
        # 计算对抗时间前的碰撞率
        self._df.set_index(["scene_index","track_id","type","ts"])
        other_df = self._df[~self._df["track_id"].isin([adv_index, 0])]
        coll_whole_horizon = other_df[other_df["ts"] <= adv_time].groupby(["scene_index","track_id","type"])["met"].max()
        coll_data = coll_whole_horizon[coll_whole_horizon.index.isin([-1],level=2)]
        other_coll_any = coll_data.groupby(["scene_index"]).mean().to_numpy()
        return {
            "coll_any": other_coll_any,
        }
    ...
    def get_sol_episode_metrics(self, adv_index):
        ...
        self._df.set_index(["scene_index","track_id","type","ts"])
        other_df = self._df[~self._df["track_id"].isin([adv_index, 0])]
        coll_whole_horizon = other_df.groupby(["scene_index","track_id","type"])["met"].max()
        coll_data = coll_whole_horizon[coll_whole_horizon.index.isin([-1],level=2)]
        other_coll_any = coll_data.groupby(["scene_index"]).mean().to_numpy()
        return {
            "coll_any": other_coll_any,
...
class CriticalFailure(EnvMetrics):
    ...
    def get_adv_episode_metrics(self, adv_index, adv_time):
        other_df = self._df[~self._df["track_id"].isin([adv_index, 0])]
        ohter_df = other_df[other_df["ts"] <= adv_time]
        coll_fail_cases = ohter_df.groupby(["scene_index","track_id"])["collision"].any()
        offroad_fail_cases = ohter_df.groupby(["scene_index","track_id"])["offroad"].any()
        coll_by_scene = coll_fail_cases.groupby(["scene_index"])
        coll_fail_rate = (coll_by_scene.sum()/coll_by_scene.count()).to_numpy()
        offroad_by_scene = offroad_fail_cases.groupby(["scene_index"])
        offroad_fail_rate = (offroad_by_scene.sum()/offroad_by_scene.count()).to_numpy()
        ...
        any_fail_by_scene = any_fail_cases.groupby(["scene_index"])
        any_fail_rate = (any_fail_by_scene.sum()/any_fail_by_scene.count()).to_numpy()
        return dict(failure_offroad=offroad_fail_rate,failure_collision=coll_fail_rate,failure_any=any_fail_rate)
    
    def get_sol_episode_metrics(self, adv_index):
        other_df = self._df[~self._df["track_id"].isin([adv_index, 0])]
        coll_fail_cases = other_df.groupby(["scene_index","track_id"])["collision"].any()
        coll_by_scene = coll_fail_cases.groupby(["scene_index"])
        coll_fail_rate = (coll_by_scene.sum()/coll_by_scene.count()).to_numpy()
        offroad_fail_cases = other_df.groupby(["scene_index","track_id"])["offroad"].any()
        offroad_by_scene = offroad_fail_cases.groupby(["scene_index"])
        offroad_fail_rate = (offroad_by_scene.sum()/offroad_by_scene.count()).to_numpy()
        any_fail_cases = coll_fail_cases | offroad_fail_cases
        any_fail_by_scene = any_fail_cases.groupby(["scene_index"])
...
class Comfort(EnvMetrics):
    ...
    def get_sol_episode_metrics(self, adv_index):
        state_traj = np.stack(self._per_step, axis=1) # num_agents x T x 2
        state_traj = state_traj[:1, :adv_index]
        # downsample
        dt_ratio = int(math.ceil(self.stat_dt / self.sim_dt))
        state_traj = state_traj[:,::dt_ratio]
