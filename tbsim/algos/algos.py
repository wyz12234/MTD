...
class BehaviorCloning(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        metrics = self._compute_metrics(pout, batch)
        for k in losses:
            m = torch.stack([losses[k]]).mean()
            self.log("val/losses_" + k, m)
        for k in metrics:
            m = np.stack([metrics[k]]).mean()
...
class DiffuserTrafficModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for k in return_dict["losses"]:
            m = torch.stack([return_dict["losses"][k]]).mean()
            self.log("val/losses_" + k, m)
        for k in return_dict["metrics"]:
            m = np.stack([return_dict["metrics"][k]]).mean()
            self.log("val/metrics_" + k, m)
        
        if self.use_ema:
            for k in return_dict["ema_losses"]:
                m = torch.stack([return_dict["ema_losses"][k]]).mean()
                self.log("val/ema_losses_" + k, m)
            for k in return_dict["ema_metrics"]:
                m = np.stack([return_dict["ema_metrics"][k]]).mean()
...
class STRIVETrafficModel(pl.LightningModule):
    def __init__(self, algo_config, modality_shapes):
        ...
        self.choose_z_stack_tensor = None
        self.choose_z_stack_tensor_list = None
        self.is_matchplanner = True
        self.find_sol = False
    ...
    def get_action(self, obs_dict, num_action_samples=1,
        ...
        if kwargs['step_index'] == 0 and (not self.find_sol) and self.is_matchplanner:
            self.choose_z_stack_tensor_list = None
        elif kwargs['step_index'] == 0 and self.find_sol:
            self.is_matchplanner = False
        obs_dict = dict(obs_dict)
        obs_dict['step_index'] = kwargs['step_index']
        ...
        preds, z = self.nets["policy"].sample(obs_dict, n=num_action_samples,
                                            guide_as_filter_only=guide_as_filter_only, choose_z_stack_tensor=self.choose_z_stack_tensor_list)
        guide_losses = preds.pop("guide_losses", None)                
        ...
        choose_z = TensorUtils.map_tensor(z, lambda x: x[torch.arange(B), act_idx]).unsqueeze(0)
        self.choose_z_stack_tensor = torch.cat([self.choose_z_stack_tensor, choose_z], dim=0) if self.choose_z_stack_tensor is not None else choose_z

        if kwargs['step_index'] == 95 and self.is_matchplanner:
            self.choose_z_stack_tensor_list = self.choose_z_stack_tensor
            self.choose_z_stack_tensor = None
            self.is_matchplanner = False

        elif kwargs['step_index'] == 95 and not self.is_matchplanner:
            self.choose_z_stack_tensor_list = self.choose_z_stack_tensor
            self.choose_z_stack_tensor = None
            self.is_matchplanner = True
            self.find_sol = False
...
class SceneDiffuserTrafficModel(pl.LightningModule):
    ...
    def forward(self, obs_dict, plan=None, step_index=0, num_samp=1, class_free_guide_w=0.0, guide_as_filter_only=False, guide_clean=False, global_t=0):
        if self.disable_control_on_stationary and global_t == 0:
            self.stationary_mask = get_stationary_mask(obs_dict, self.disable_control_on_stationary, self.moving_speed_th) # 屏蔽掉静止的agent
