...
class STRIVEPrior(Prior):
    def __init__(
        ...
        super(STRIVEPrior, self).__init__(latent_dim)
        self.trajectory_shape = trajectory_shape
        output_shapes = self.posterior_param_shapes
    ...
    def kl_loss(self, posterior_params, inputs=None):
        ...
        batch_size = posterior_params["mu"].shape[0]
        prior_params = self.forward(inputs)
        return KLD_gaussian_loss(
            mu_1=posterior_params["mu"],
            logvar_1=posterior_params["logvar"],
            mu_2=prior_params["mu"],
...
class CVAE(nn.Module):
    ...
    def sample_q_params(self, data_batch, cond_feat):
        trajectories = torch.cat([torch.nan_to_num(data_batch["target_positions"], nan=0.0), torch.nan_to_num(data_batch["target_yaws"], nan=0.0)], dim=-1)
        inputs = OrderedDict(trajectories=trajectories)
        q_params = self.q_net(inputs=inputs, condition_features=cond_feat)
