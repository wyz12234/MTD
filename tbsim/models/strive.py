...
from tbsim.utils.guidance_loss import DiffuserGuidance, verify_guidance_config_list, MapCollisionLoss
...
class STRIVEVaeModel(nn.Module):
    def __init__(self, algo_config, modality_shapes, weights_scaling):
        ...
        prior = vaes.STRIVEPrior(condition_dim=algo_config.vae.condition_dim,
        ...
        self.map_collision_loss = MapCollisionLoss()
    ...
    def sample(self, batch_inputs: dict, n: int,
                guide_as_filter_only=False, choose_z_stack_tensor=None):
        condition_inputs = batch_inputs #OrderedDict(image=batch_inputs["image"], goal=None)
        ...
        if self.current_guidance is not None:
            ...
            outs, guide_losses, z = self.guidance_optim(condition_inputs, n, decoder_kwargs, batch_inputs,
                                                        num_iter=0 if guide_as_filter_only else 300, choose_z_stack_tensor=choose_z_stack_tensor)
        else:
    ...
    def compute_losses(self, pred_batch, data_batch):
        ...
        offroad_loss = self.map_collision_loss(pred_batch["trajectories"].unsqueeze(1), data_batch).mean()
        collision_loss = self.agent_loss(pred_batch, data_batch)

        losses = OrderedDict(prediction_loss=pred_loss, kl_loss=kl_loss, offroad_loss=offroad_loss, collision_loss=collision_loss)
        if self.dyn is not None:
            losses["yaw_reg_loss"] = torch.mean(pred_batch["controls"][..., 1] ** 2)
        return losses

    def agent_loss(self, pred_batch, data_batch):
        # compute distance between agent and other agents
        distance = torch.norm(pred_batch["trajectories"].unsqueeze(1)[..., :2] - data_batch["all_other_agents_future_positions"], dim=-1)
        penalty_dist = data_batch['all_other_agents_extents'][:, :, :2].mean(dim=-1, keepdim=True) + data_batch['agent_fut_extent'][:, :1, :2].mean(dim=-1, keepdim=True) + 0.2
        penalty_mask = distance < penalty_dist
        penalty_mask = penalty_mask & data_batch['all_other_agents_future_availability']
        cur_penalties = 1.0 - (distance / penalty_dist)
        cur_penalties = torch.where(penalty_mask,
                                    cur_penalties,
                                    torch.zeros_like(cur_penalties))
        return cur_penalties.sum(dim=1).mean()
    ...
    def guidance_optim(self, condition_inputs, num_samp, decoder_kwargs, data_batch,
        ...
        with torch.enable_grad():
            ...
            q_params = self.vae.sample_q_params(data_batch, cond_feat)
            prior_mean = q_params['mu'].unsqueeze(1).expand_as(init_z).detach()
            prior_logvar = q_params['logvar'].unsqueeze(1).expand_as(init_z).detach()
            ...
            for oidx in pbar_optim:
                ...
                prior_loss = torch.mean(-log_normal(z, prior_mean, prior_var)) * 1e-2
                if choose_z_stack_tensor is not None:
                    init_loss = torch.mean(torch.sum((z - choose_z_stack_tensor[data_batch['step_index'] // 5].unsqueeze(1)) ** 2, dim=2), dim=1)
                    init_weight = data_batch.get('init_weight', None)
                    if init_weight is not None:
                        init_weight[0] = init_weight[0] * 0.1
                        init_loss = torch.sum(init_weight * init_loss * 0.1)
                    else:
                        init_loss[0] = init_loss[0] * 0.1
                        init_loss = torch.sum(init_loss * 0.1 / init_loss.size(0))
                    loss = loss + init_loss
            ...
            return final_out, guide_losses, z
