...
class SceneDiffuserModel(nn.Module):
    ...
    def __init__(
        ...
        # ------ basic dimensions ------
        self.dt = dt # 0.1
        self.horizon = horizon # 52
        self.hist_num_frames = hist_num_frames # 31
        
        self.observation_dim = observation_dim # 4
        self.action_dim = action_dim # 2
        self.transition_dim = observation_dim + action_dim # 6
        self.output_dim = output_dim # 2

        self.hist_feature_dim = hist_feature_dim # 64

        # ------ architectures ------
        # ['TemporalMapUnet', 'SceneTransformer']
        self.diffuser_model_arch = diffuser_model_arch # 'SceneTransformer'
        # decide how the context is integrated
        self.agent_hist_embed_method = agent_hist_embed_method # 'concat'
        self.neigh_hist_embed_method = neigh_hist_embed_method # 'interaction_edge'
        self.map_embed_method = map_embed_method # 'transformer'
        self.interaction_edge_speed_repr = interaction_edge_speed_repr # 'rel_vel_new_new'
        self.normalize_rel_states = normalize_rel_states # True
        # this field is used only when SceneTransformer is used
        self.mask_social_interaction = mask_social_interaction # False
        # this field is used only when agent_hist_embed_method == 'mlp' or map_embed_method == 'cnn'
        self.single_cond_feat = single_cond_feat # False
        self.mask_edge = mask_edge # False
        self.neighbor_inds = neighbor_inds # [0,1,2,3,4,5,9,10] 边中使用的信息索引对应于(x,y,cos,sin,speed,l,w)或(x,y,cos,sin,velx,vely,l,w,rel_d,rel_d_lw, rel_t_to_collision)
        self.edge_attr_separation = edge_attr_separation # [[0,1],[2,3],[4,5],[6,7]] 边属性分离
        self.social_attn_radius = social_attn_radius # 30
        self.use_last_hist_step = use_last_hist_step # False
        self.use_noisy_fut_edge = use_noisy_fut_edge # False
        self.use_const_speed_edge = use_const_speed_edge # True
        self.all_interactive_social = all_interactive_social # False
        self.mask_time = mask_time # False
        self.layer_num_per_edge_decoder = layer_num_per_edge_decoder # 4
        self.attn_combination = attn_combination # 'gate'

        self.disable_control_on_stationary = disable_control_on_stationary # 'current_speed'
        self.stationary_mask = None
        self.coordinate = coordinate # 'agent_centric'
        ...
        self.use_conditioning = use_conditioning # True
        # for test-time classifier-free guidance, if desired
        self.cond_fill_value = cond_fill_value # 0.5

        self.rasterized_map = rasterized_map # False
        self.rasterized_hist = rasterized_hist # False
        ...
        self._dynamics_type = dynamics_type # 'Unicycle'
        ...
        self.action_loss_only = action_loss_only # False
        ...
        self.use_reconstructed_state = use_reconstructed_state # False
        self.diffuser_input_mode = diffuser_input_mode # 'state_and_action'
        
        if diffuser_input_mode == 'state':
            self.default_chosen_inds = [0, 1, 3]
        elif diffuser_input_mode == 'action': # default
        ...
        self.loss_fn = Losses[loss_type](recon_loss_weights, self.action_dim, loss_decay_rates) # WeightedL2WithCollision
    ...
    def compute_losses(self, data_batch):
        aux_info = self.get_aux_info(data_batch)  # encoding, containing agent_hist, neighbor_hist, map_feat
    ...
    def p_mean_variance(self, x, t, aux_info={}, class_free_guide_w=0.0, data_batch=None, x_clean_model_out=None):
        ...
        if self.neigh_hist_embed_method in ['interaction_edge', 'interaction_edge_and_input']:
            ...
            aux_info = self.get_neighbor_future_relative_states(x_edge, aux_info, data_batch) # 最后一帧hist的恒定速度模型
    ...
    def p_losses(self, x_start_init, t, data_batch, aux_info={}):
        ...
        if self.neigh_hist_embed_method in ['interaction_edge', 'interaction_edge_and_input']:
            if self.use_noisy_fut_edge:
                ...
                x_edge = x_start_init.detach().clone() # 原始轨迹
            aux_info = self.get_neighbor_future_relative_states(x_edge, aux_info, data_batch) # 使用恒定速度模型给邻居添加未来信息
