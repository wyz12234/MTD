...
from configs.strive_edit_config import STRIVEEditingConfig
    ...
    while scene_i < eval_cfg.num_scenes_to_evaluate:
        ...
        for ei in range(eval_cfg.num_sim_per_scene):
            ...
            if eval_cfg.eval_class == "STRIVE":
                ...
                match_config = compute_heuristic_guidance(match_config,
                                                            env,
                                                            sim_scene_indices,
                                                            sim_start_frames,
                                                            example_batch=ex_obs['agents'])
            ...
            coll_time = env._metrics['all_collision_rate'].coll_ts
            coll_index = env._metrics['all_collision_rate'].coll_index
            
            adv_metrics = {}
            if coll_time is None:
                # 计算离ego最近的车辆坐标
                all_positions = info["buffer"][0]["centroid"]
                ego_position = all_positions[0]
                agents_positions = all_positions[1:]
                min_distance = None
                for i, agent_position in enumerate(agents_positions):
                    for t in range(len(agent_position)):
                        distance = np.linalg.norm(ego_position[t] - agent_position[t])
            ...
            adv_metrics['other_coll_any'] = env._metrics['all_collision_rate'].get_adv_episode_metrics(coll_index, coll_time)['coll_any']
            for k, v in env._metrics['all_failure'].get_adv_episode_metrics(coll_index, coll_time).items():
                adv_metrics['other_' + k] = v
            for k, v in env._metrics['all_off_road_rate'].get_adv_episode_metrics(coll_index, coll_time).items():
                adv_metrics['offroad_' + k] = v
        ...
        if render_to_video or render_to_img:
            ...
            for si, scene_buffer in zip(info["scene_index"], info["buffer"]):
                viz_dir = os.path.join(eval_cfg.results_dir, "viz/", "success" if veh_coll_list[scene_cnt] else "fail")
        ...
        # if success try to find a solution
        if any(veh_coll_list):
            print('Adv gen succeeded! Finding solution...')
            sol_info = {}
            sol_stats = {}
            if eval_cfg.apply_guidance:
                if eval_cfg.eval_class in ['QueryDiffuser', 'SceneDiffuser']:
                    policy_model.set_guidance_optimization_params(eval_cfg.sol_guidance_optimization_params)
                    policy_model.set_diffusion_specific_params(eval_cfg.diffusion_specific_params)

            for ei in range(eval_cfg.num_sim_per_scene):
                if not veh_coll_list[ei]:
                    continue
                ...
                if "heuristic" in eval_cfg.edits.editing_source:
                    ...
                    # build heuristic guidance configs for these scenes
                    heuristic_guidance_cfg = compute_heuristic_guidance(heuristic_sol_config,
                                                                        env,
                                                                        sim_scene_indices,
                                                                        sim_start_frames,
                                                                        example_batch=ex_obs['agents'])
                ...
                if eval_cfg.eval_class == "STRIVE":
                    policy_model.find_sol = True

                metrics, env_info, renderings, save_obs, added_metrics = guided_rollout(
                    env,
                    rollout_policy,
                    policy_model,
                    n_step_action=eval_cfg.n_step_action,
                    ...
                    eval_class=eval_cfg.eval_class,
                    apply_guidance=eval_cfg.apply_guidance,
                    guided_pos=info["buffer"][ei]["centroid"],
                    guided_yaw=info["buffer"][ei]["yaw"],
                )
                is_batched_env = isinstance(env, BatchedEnv)
                for k, v in env_info.items():
                    if k not in sol_info:
                        sol_info[k] = []
                    if is_batched_env:
                        sol_info[k].extend(v)
                    else:
                ...
                for k, v in metrics.items():
                    if k not in sol_stats:
                        sol_stats[k] = []
                    if is_batched_env:  # concatenate by scene
                        sol_stats[k] = np.concatenate([sol_stats[k], v], axis=0)
                    else:
                        sol_stats[k].append(v)

                # remove all temporary added metrics
                for met_name in added_metrics:
                    env._metrics.pop(met_name)

                coll_index = coll_index_list[ei]
                
                sol_metrics = {}
                sol_metrics['other_coll_any'] = env._metrics['all_collision_rate'].get_sol_episode_metrics(coll_index)['coll_any']
                for k, v in env._metrics['all_failure'].get_sol_episode_metrics(coll_index).items():
                    sol_metrics['other_' + k] = v
                for k, v in env._metrics['all_off_road_rate'].get_sol_episode_metrics(coll_index).items():
                    sol_metrics['offroad_' + k] = v
                ...
                veh_coll_sol = True
                coll_time_sol = env._metrics['all_collision_rate'].coll_ts
                if coll_time_sol is None:
                    veh_coll_sol = False
                veh_coll_sol_list.append(veh_coll_sol)
                if not veh_coll_sol:
                    print('Solution found!')
                    # save_obs_list.append(save_obs)
                    if sol_stats["offroad_ego_rate"][-1] < 0.1:
                        if eval_cfg.eval_class == "QueryDiffuser":
                            with open(eval_cfg.training_data_dir, 'ab') as file:
                                pickle.dump(save_obs, file)
                    if data_to_disk and "buffer" in sol_info:
                        dump_episode_buffer(
                            [sol_info["buffer"][-1]],
                            [sol_info["scene_index"][-1]],
                            start_frame_index[0][ei: ei+1],
                            h5_path=eval_cfg.experience_sol_hdf5_path
                        )
                torch.cuda.empty_cache()
            multi_episodes_metrics = env.get_multi_episode_metrics()
            sol_stats.update(multi_episodes_metrics)
            env.reset_multi_episodes_metrics()

            print(sol_info["scene_index"])
            print(sim_start_frames)
            pprint(sol_stats)
            ...
            for k,v in sol_stats.items():
                if k.split('_')[0] == 'guide':
                    ...
                    pop_list.append(k)
            for k in pop_list:
                sol_stats.pop(k, None)
            # average over all of the same guide stats in each scene
            for k,v in guide_agg_dict.items():
                scene_stats = np.stack(v, axis=0) # guide_per_scenes x num_scenes (all are nan except 1)
                sol_stats[k] = np.mean(scene_stats, axis=0)

            # aggregate metrics stats
            if sol_result_stats is None:
                sol_result_stats = sol_stats
                sol_result_stats["scene_index"] = np.array(info["scene_index"])
            else:
                for k in sol_stats:
                    if k not in sol_result_stats:
                        sol_result_stats[k] = sol_stats[k]
                    else:
                        sol_result_stats[k] = np.concatenate([sol_result_stats[k], sol_stats[k]], axis=0)
                sol_result_stats["scene_index"] = np.concatenate([sol_result_stats["scene_index"], np.array(info["scene_index"])])

            # write stats to disk
            with open(os.path.join(eval_cfg.results_dir, "sol_stats.json"), "w+") as fp:
                stats_to_write = map_ndarray(sol_result_stats, lambda x: x.tolist())
                json.dump(stats_to_write, fp)

            
            # sol_pos, sol_yaw = run_find_solution_optim(info["buffer"][0], ego_traj, agents_traj, ego_lw, agents_lw, coll_time, min_index, first_obs) # old

            if render_to_video or render_to_img:
                # info_new = deepcopy(info)
                # info_new["buffer"][0]["centroid"][0][1: coll_time + 10] = sol_pos
                # info_new["buffer"][0]["yaw"][0][1: coll_time + 10] = sol_yaw
                # info_new["buffer"][0]["centroid"] = info_new["buffer"][0]["centroid"][:, :coll_time + 10]
                # info_new["buffer"][0]["yaw"] = info_new["buffer"][0]["yaw"][:, :coll_time + 10]
                # high quality
                from tbsim.utils.scene_edit_utils import visualize_guided_rollout
                scene_cnt = 0
                for si, scene_buffer in zip(sol_info["scene_index"], sol_info["buffer"]):
                    viz_dir = os.path.join(eval_cfg.results_dir, "viz_sol/", "success" if not veh_coll_sol_list[scene_cnt] else "fail")
                    invalid_guidance = guidance_config is None or len(guidance_config) == 0
                    invalid_constraint = constraint_config is None or len(constraint_config) == 0
                    visualize_guided_rollout(viz_dir, render_rasterizer, si, scene_buffer,
                                                guidance_config=None if invalid_guidance else guidance_config[0],
                                                ...
                                                viz_diffusion_steps=False,
                                                first_frame_only=render_to_img,
                                                sim_num=[num for num, flag in zip(start_frame_index[0], veh_coll_list) if flag][scene_cnt],
                                                save_every_n_frames=render_cfg['save_every_n_frames'],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results_root_dir",
        type=str,
        default="safetycritic_scenarios",
    ...
    # cfg = STRIVEEditingConfig(registered_name=args.registered_name)
    ...
    if args.training_data_dir is not None:
        cfg.training_data_dir = args.training_data_dir
