...
def main(cfg, auto_remove_exp_dir=False, debug=False):
    ...
    if cfg.env.name == "trajdata":
        set_global_batch_type("trajdata")
        set_global_trajdata_batch_env(cfg.train.trajdata_source_train[0]) # "nusc_trainval-train"
    ...
    # sample_one = datamodule.train_dataset[0] # for test
    ...
    if debug:
        ...
        wandb.login()
        logger = WandbLogger(
            name=None, project=cfg.train.logging.wandb_project_name, entity='wyz12234'
        )
...
if __name__ == "__main__":
    ...
    if args.config_name is not None: # Example: trajdata_nusc_scene_diff
    ...
    if args.debug:
        # Test policy rollout
        default_config.train.validation.every_n_steps = 100
        default_config.train.save.every_n_steps = 500
        default_config.train.rollout.every_n_steps = 5
