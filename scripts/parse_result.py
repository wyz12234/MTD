...
def parse_single_result(results_file, results_dir, eval_out_dir, save_hist_data=False, gt_hist_path=None):
    if results_file == 'data.hdf5':
        rjson = json.load(open(os.path.join(results_dir, "stats.json"), "r"))
    else:
        rjson = json.load(open(os.path.join(results_dir, "sol_stats.json"), "r"))
    ...
    if args.estimate_dist:
        ...
        hjson = compute_and_save_hist(os.path.join(results_dir, results_file), eval_out_dir, save_hist_data, normalization_by=args.normalization_by, disable_control_on_stationary=args.disable_control_on_stationary)
        ...
        if gt_hist_path is None:
            ...
            if cfg["env"] == "trajdata":
                ...
                if 'lyft' in cfg["trajdata_source_test"][0]:
                    ...
                    gt_hjson = json.load(open("gt_files/gt_hist.json", "r"))
                else:
                    raise
            else:
                raise
    ...
    with open(os.path.join(eval_out_dir, 'results_per_scene.csv'), 'w') as f:
        ...
        for sidx, scene in enumerate(list(set(scene_results["names"]))):
...
def parse(args):
    ...
    for result_path in all_results:
        if os.path.isdir(result_path):
            ...
            agg_res = parse_single_result(results_file, result_path, eval_out_dir, args.save_hist_data, gt_hist_path=args.gt_hist)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results_dir",
        type=str,
        default='/home/wyz/diffusion_trajection/nusc_results_exp1/query_edit_eval',
