...
def rename_file_with_sequence(directory, file_name, start_sequence):
    file_path = os.path.join(directory, file_name)
    
    if os.path.exists(file_path):
        base, ext = os.path.splitext(file_name)
        sequence = start_sequence
        new_file_name = f"{base}_{sequence}{ext}"
...
def run_scene_editor(eval_cfg, save_cfg, data_to_disk, render_to_video, render_to_img, render_cfg):
    ...
    rename_cnt = 1
    try:
        shutil.rmtree(os.path.join("viz_exp1/", eval_cfg.name))
        os.makedirs(os.path.join("viz_exp1/", eval_cfg.name))
    except:
        os.makedirs(os.path.join("viz_exp1/", eval_cfg.name), exist_ok=True)
    ...
    while scene_i < eval_cfg.num_scenes_to_evaluate:
        ...
        scene_indices = eval_scenes[98: 98 + eval_cfg.num_scenes_per_batch]
        ...
        if render_to_video or render_to_img:
            ...
            for si, scene_buffer in zip(info["scene_index"], info["buffer"]):
                viz_dir = os.path.join("viz_exp1/", eval_cfg.name)
            ...
            if file_name is None:
                file_name = get_first_file_name(viz_dir)
            rename_file_with_sequence(viz_dir, file_name, rename_cnt)
            rename_cnt += 1
